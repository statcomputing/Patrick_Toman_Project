...
if(gev){
  ...
  boots <- foreach(i = 1:iteration,.packages="quantreg") %dopar% {
    ...
    if(stationary){
      ...
      return_boot <- model3_gev_nlme(resample_list,boot_list,stationary = T)
      
    }else{
      
      return_boot <- model3_gev_nlme(resample_list,boot_list,stationary = F) 
...
for(i in 1:num_return_years){
  ...
  zBC[1,i] <- qnorm(sum(return_boot_midway[,i] < stationary_mod3$returns[1,i])/iteration,mean=0,sd=1)
  
  zBC[2,i] <- qnorm(sum(return_boot_ohare[,i] < stationary_mod3$returns[2,i])/iteration,mean=0,sd=1)
  
  zBC[3,i] <- qnorm(sum(return_boot_parkforest[,i] < stationary_mod3$returns[3,i])/iteration,mean=0,sd=1)
...
if(gev){
  ...
  jacks <- foreach(i = 1:num_years,.packages="quantreg") %dopar% {
    ...
    if(stationary){
      ...
      return_jack <- model3_gev_nlme(jack_sample_list,jack_year,stationary = T)
      
    }else{
      
      return.jack <- model3_gev_nlme(jack_sample_list,jack_year,stationary = F)
